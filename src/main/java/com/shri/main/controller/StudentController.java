...
import com.shri.main.service.StudentHistoryService;
...
//	}
	...
	public String saveStudent(@ModelAttribute Student student) {
	    ...
	    student.setUpdatedOn(LocalDate.now().toString()); 
	    stdDao.save(student);
	    StudentHistory history = new StudentHistory();
	    history.setStudentId(student.getId());
	    history.setUpdatedOn(student.getUpdatedOn());
	    history.setPaidFees(student.getPaidFees());
	    history.setComments(student.getComments());
        studentHistoryRepo.save(history);
        logger.info("Record saved successfully & history updated.!");
	    return "redirect:/list";
	}
	...
	public ModelAndView showUpdateForm(@RequestParam Long studentId) {
	    ...
	    if (studentdata != null) {
	        ...
	        logger.info("Birthday Wish Sent: {}", Boolean.TRUE.equals(studentdata.getIsBirthdayWishSent()));
...
//	
	...
	public ModelAndView ShowStudentSecureData(@RequestParam Long studentId) {
	    ModelAndView modelAndView = new ModelAndView("ShowStudentSecureData");
        Student studentdata = stdDao.findById(studentId).orElse(null);
        List<StudentHistory> previousRecords = studentHistoryService.getHistoryByStudentId(studentId);
	    modelAndView.addObject("student", studentdata);
	    modelAndView.addObject("previousRecords", previousRecords);
	    logger.info("Record Viewed successfully.!");
	    return modelAndView;
	}
